...
- Keep responses concise: lead with the answer; skip preamble and recaps.
...
- When navigating code semantically, prefer the LSP tool (load it via ToolSearch if deferred) for go-to-definition, find-references, implementations of an interface, and call hierarchies. The GoLand MCP tools (mcp__goland__search_symbol, get_symbol_info, analyze_calls, etc.) are an equivalent alternative when that server is connected. Use Grep only for text patterns neither can express.
