...
description: Prepare a principal-software-engineer-grade plan for responding to the review comments on a pull request. Takes a GitHub PR link (or bare PR number for the current repo) as the first argument; if omitted, infers the PR from the current branch via `gh pr view` and confirms via AskUserQuestion. Fetches the PR's review threads, reviews, and diff via the gh CLI, identifies the associated Jira ticket (from the branch name, PR title/body, or commit messages; confirmed via AskUserQuestion when ambiguous) and fetches it via the Atlassian MCP tools, then analyzes the codebase, the tests, and the spec files (specs/<name>/ and other design docs) touched by or relevant to each comment — fanning the per-comment-cluster analysis out across parallel general-purpose subagents launched concurrently. Produces a per-thread plan — classification (agree / agree-with-modification / push back / needs clarification), evidence from code/tests/specs/ticket, proposed action with file references, a drafted reply, and sequencing — and writes it to review_response_<PR#>.md in the current directory (updating an existing plan in place on rerun). After writing, runs `/codex:adversarial-review --background` on the plan, validates every codex finding against the PR and repo evidence (adjudicated in parallel subagent batches), and folds accepted findings back into the plan — routing judgment calls through AskUserQuestion. The deliverable is the plan file only: this skill never posts replies to the PR, never pushes code, and never modifies the working tree beyond the plan file. Invoke manually when a PR has review comments you need to respond to.
---
