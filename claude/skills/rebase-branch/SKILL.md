---
name: rebase-branch
description: Rebase the current branch onto an input branch so the input's changes land here with linear history (the input branch is never modified), then reconcile the current branch's code, tests, and docs with the incoming changes, codex-review the result, and verify with the project's format/lint/build/test gates. Takes a local branch name or remote ref (e.g. origin/main) as the argument; resolves conflicts itself, confirming risky resolutions first. Invoke manually with the input branch name, e.g. to bring main into your feature branch.
---
