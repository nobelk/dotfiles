...
- **Delegate to a `general-purpose` subagent** (each returns a compact result, keeping verbose output out of the main context):
  - **Step 2** — launch the `/codex:review --background` review, poll `/codex:status` to completion **under the 10-minute deadline / 5-minute-silence stall rule defined in Step 2** (cancel + one fast-model retry on breach), fetch `/codex:result <job-id>`, and return the raw findings verbatim (also written to the scratch file). The lengthy codex transcript stays in the subagent.
...
- **Enforce a 10-minute deadline.** The companion has no internal timeout — a stalled `review/start` turn hangs the job forever (observed: 3.5 min of activity, then 38 min of silence). Poll with `node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" status <job-id> --wait` (4-min bounded waits) and track elapsed time. Additionally, check the job's `logFile` timestamp between polls: if the log has been silent for **5+ minutes** while status still says running, treat the job as stalled even before the 10-minute mark. On deadline or stall: run `/codex:cancel <job-id>`, then retry **once** with the fast reviewer model (`node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" review "--background --model gpt-5.4-mini <same scope flags>"` — must be a model the account supports; `gpt-5.x-codex` models are rejected on ChatGPT-account logins). If the retry also exceeds the deadline, cancel it and fall through to the codex-failed AskUserQuestion below.
